...
def multiplication(bob: Bob, alice: Alice) -> int:
    ...
    # Alice
    def alice_mult(c_rx: int, c_sy: int) -> int:
        # Ici on envoie la 0 knowledge proof
    ...
    def mult2(c_rx: int, c_sy: int) -> int:
        c_rx_sy = alice_mult(c_rx, c_sy)
        delta = gen_random_znz(alice.pk)
        pi = Paillier.MulConst(c_sy, delta, alice.pk)
        return c_rx, c_sy, c_rx_sy, delta, pi
